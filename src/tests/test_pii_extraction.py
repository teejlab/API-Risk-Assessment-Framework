...
def test_wrong_parameters():
    ...
    # Test if the function raises an error when 
    # the confidence threshold is not a float or an int
    with raises(TypeError):
        pii_extraction("raw_text", type="pii", conf_threshold='123')

    # Test if the function raises an error when 
    # the confidence threshold is not between 0 and 1
    with raises(ValueError):
...
def test_json():
    results = pii_extraction("{email:'abc@test.com'}",
                             type="pii",
                             conf_threshold=0.5)
...
def test_html():
    results = pii_extraction("<html><body>Test123</body></html>",
                             type="pii",
                             conf_threshold=0.5)
